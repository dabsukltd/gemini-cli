...
describe('loadExtensions', () => {
  ...
  it('should load context file path when PHOENIX.md is present', () => {
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'PHOENIX.md'),
    ]);
...
function createExtension(
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'PHOENIX.md'), 'context');
  }
