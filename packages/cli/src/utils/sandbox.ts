...
import { SandboxConfig } from '@phoenix-ignite/phoenix-cli-core';
...
export async function start_sandbox(
  ...
  if (config.command === 'sandbox-exec') {
    ...
    // start and set up proxy if PHOENIX_SANDBOX_PROXY_COMMAND is set
    const proxyCommand = process.env.PHOENIX_SANDBOX_PROXY_COMMAND;
    ...
    if (!gcPath.includes('gemini-cli/packages/')) {
      ...
      execSync(
        `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
        {
          ...
          env: {
            ...process.env,
            PHOENIX_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
          },
        },
      );
    }
  ...
  // PHOENIX_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
  const proxyCommand = process.env.PHOENIX_SANDBOX_PROXY_COMMAND;
