...
async function getPhoenixMdFilePathsInternal(
  ...
  const phoenixMdFilenames = getAllPhoenixMdFilenames();

  for (const phoenixMdFilename of phoenixMdFilenames) {
    ...
    if (debugMode)
      logger.debug(
        `Searching for ${phoenixMdFilename} starting from CWD: ${resolvedCwd}`,
      );
    ...
    try {
      ...
      if (debugMode)
        logger.debug(
          `Found readable global ${phoenixMdFilename}: ${globalMemoryPath}`,
        );
    } catch {
      if (debugMode)
        logger.debug(
          `Global ${phoenixMdFilename} not found or not readable: ${globalMemoryPath}`,
        );
    }
    ...
    while (currentDir && currentDir !== path.dirname(currentDir)) {
      ...
      if (debugMode) {
        logger.debug(
          `Checking for ${phoenixMdFilename} in (upward scan): ${currentDir}`,
        );
      }

      // Skip the global .phoenix directory itself during upward scan from CWD,
      // as global is handled separately and explicitly first.
      if (currentDir === path.join(resolvedHome, PHOENIX_CONFIG_DIR)) {
        if (debugMode) {
          logger.debug(
      ...
      const potentialPath = path.join(currentDir, phoenixMdFilename);
      try {
        ...
        if (potentialPath !== globalMemoryPath) {
          ...
          if (debugMode) {
            logger.debug(
              `Found readable upward ${phoenixMdFilename}: ${potentialPath}`,
            );
          }
        }
      } catch {
        if (debugMode) {
          logger.debug(
            `Upward ${phoenixMdFilename} not found or not readable in: ${currentDir}`,
          );
        }
      }
    ...
    if (debugMode && downwardPaths.length > 0)
      logger.debug(
        `Found downward ${phoenixMdFilename} files (sorted): ${JSON.stringify(
          downwardPaths,
        )}`,
      );
  ...
  if (debugMode)
    logger.debug(
      `Final ordered ${getAllPhoenixMdFilenames()} paths to read: ${JSON.stringify(
        finalPaths,
      )}`,
    );
  return finalPaths;
...
async function readPhoenixMdFiles(
  filePaths: string[],
  debugMode: boolean,
): Promise<PhoenixFileContent[]> {
  const results: PhoenixFileContent[] = [];
  for (const filePath of filePaths) {
    try {
      ...
      if (!isTestEnv) {
        ...
        logger.warn(
          `Warning: Could not read ${getAllPhoenixMdFilenames()} file at ${filePath}. Error: ${message}`,
        );
      }
...
function concatenateInstructions(
  instructionContents: PhoenixFileContent[],
...
/**
 * Loads hierarchical PHOENIX.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  const filePaths = await getPhoenixMdFilePathsInternal(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No PHOENIX.md files found in hierarchy.');
    return { memoryContent: '', fileCount: 0 };
  }
  const contentsWithPaths = await readPhoenixMdFiles(filePaths, debugMode);
