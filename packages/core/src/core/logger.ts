...
export class Logger {
  private phoenixDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.phoenixDir = getProjectTempDir(process.cwd());
    this.logFilePath = path.join(this.phoenixDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.phoenixDir, { recursive: true });
  ...
  _checkpointPath(tag: string): string {
    ...
    return path.join(this.phoenixDir, `checkpoint-${tag}.json`);
  }
