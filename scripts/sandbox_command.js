...
let geminiSandbox = process.env.PHOENIX_SANDBOX;

if (!geminiSandbox) {
  const userSettingsFile = join(os.homedir(), '.phoenix', 'settings.json');
...
if (!geminiSandbox) {
  ...
  while (true) {
    const geminiEnv = join(currentDir, '.phoenix', '.env');
  ...
  geminiSandbox = process.env.PHOENIX_SANDBOX;
}
...
if (['1', 'true'].includes(geminiSandbox)) {
  if (commandExists('docker')) {
    ...
    console.error(
      'ERROR: install docker or podman or specify command in PHOENIX_SANDBOX',
    );
    process.exit(1);
  }
  ...
  if (commandExists(geminiSandbox)) {
    ...
    console.error(
      `ERROR: missing sandbox command '${geminiSandbox}' (from PHOENIX_SANDBOX)`,
    );
    process.exit(1);
  }
} else {
